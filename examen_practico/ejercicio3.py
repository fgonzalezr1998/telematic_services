...
TimestampFields = 2
FullDateFields = 4
AllowedCities = ["Madrid", "Londres", "Moscu", "Tokio", "New_York", "Lisboa", "Berlin", "Singapur", "UTC"]
LocalHoursAllowed = ["madrid", "londres", "moscu", "tokio", "new_York", "lisboa", "berlin", "singapur"]
...
def timezone_loc(city):
    ...
    if(city == LocalHoursAllowed[5]):
        tl = pytz.timezone("Europe/Lisbon")
    if(city == LocalHoursAllowed[6]):
        tl = pytz.timezone("Europe/Berlin")
    if(city == LocalHoursAllowed[7]):
        tl = pytz.timezone("Asia/Singapore")
